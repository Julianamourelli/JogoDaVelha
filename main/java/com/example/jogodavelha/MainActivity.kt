...
class MainActivity : AppCompatActivity() {
    ...
    fun buttonClick(view: View) {
        ...
        if(!vencedor.isNullOrBlank()) {
            if (vencedor == "X") {
                Toast.makeText(this, "Vencedor:Sonserina!", Toast.LENGTH_LONG).show()
            }
            if (vencedor == "O") {
                Toast.makeText(this, "Vencedor:Grifinória!", Toast.LENGTH_LONG).show()
            }
        ...
        if(jogadorAtual.equals("X")) {
            buttonSelecionado.setBackgroundResource(R.drawable.sonserina)
            jogadorAtual = "O"
        }else{
            buttonSelecionado.setBackgroundResource(R.drawable.grifinoria)
            jogadorAtual = "X"
        }
